/***********************************************************************
    MOTEUR DE RÈGLES T-SQL - VERSION 6.2
...
PRINT '══════════════════════════════════════════════════════════════════════';
PRINT '           MOTEUR DE RÈGLES V6.2 - Spec V1.5.5                        ';
PRINT '══════════════════════════════════════════════════════════════════════';
PRINT '';
GO
...
BEGIN
    SET NOCOUNT ON;
    SET ANSI_WARNINGS OFF;  -- Supprime les avertissements NULL dans les agrégats (comportement attendu §8.4)
    ...
    BEGIN
        ...
        BEGIN
            ...
            BEGIN
                ...
                -- La règle appelante continue et recevra NULL (§8.4)
            END
            -- State = 2 (EVALUATED) ou 3 (ERROR) : ne rien faire
            FETCH NEXT FROM rule_cursor INTO @RuleKey, @RuleState;
        END
        CLOSE rule_cursor;
    ...
    ELSE IF @BaseAggregator = 'CONCAT'
        SET @SQL = N'SELECT @R = ISNULL(STRING_AGG(ScalarValue, '','') WITHIN GROUP (ORDER BY SeqId), '''') FROM #ThreadState WHERE ' + @WhereClause + ' AND ScalarValue IS NOT NULL';
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- Mode
        SET @Mode = ISNULL(OPENJSON(@InputJson, '$.mode'), 'NORMAL');
        IF @Mode NOT IN ('NORMAL', 'DEBUG') SET @Mode = 'NORMAL';
        
        -- Options
        SET @StopOnFatal = ISNULL(TRY_CAST(OPENJSON(@InputJson, '$.options.stopOnFatal') AS BIT), 0);
        SET @ReturnStateTable = ISNULL(TRY_CAST(OPENJSON(@InputJson, '$.options.returnStateTable') AS BIT), 1);
        SET @ReturnDebug = ISNULL(TRY_CAST(OPENJSON(@InputJson, '$.options.returnDebug') AS BIT), 0);
        ...
        SELECT
            ...
            v.[value],                           -- NVARCHAR(MAX), no truncation
...
PRINT '';
PRINT '══════════════════════════════════════════════════════════════════════';
PRINT '           INSTALLATION TERMINÉE - V6.2                               ';
PRINT '══════════════════════════════════════════════════════════════════════';
PRINT '';
