...
IF OBJECT_ID('dbo.fn_ResolveSimpleAggregate','FN') IS NOT NULL DROP FUNCTION dbo.fn_ResolveSimpleAggregate;
...
CREATE OR ALTER PROCEDURE dbo.sp_ResolveSimpleAggregate
...
BEGIN
    ...
    DECLARE @Agg VARCHAR(20) = UPPER(@Aggregator);
    DECLARE @R   NVARCHAR(MAX) = NULL;

    IF @Agg IN ('SUM','AVG','MIN','MAX','COUNT')
    BEGIN
        SELECT @R = CASE @Agg
            WHEN 'SUM'   THEN CAST(SUM(TRY_CAST(ScalarValue AS DECIMAL(38,10))) AS NVARCHAR(MAX))
            WHEN 'AVG'   THEN CAST(AVG(TRY_CAST(ScalarValue AS DECIMAL(38,10))) AS NVARCHAR(MAX))
            WHEN 'MIN'   THEN CAST(MIN(TRY_CAST(ScalarValue AS DECIMAL(38,10))) AS NVARCHAR(MAX))
            WHEN 'MAX'   THEN CAST(MAX(TRY_CAST(ScalarValue AS DECIMAL(38,10))) AS NVARCHAR(MAX))
            WHEN 'COUNT' THEN CAST(COUNT(1) AS NVARCHAR(MAX))
        END
        FROM #ThreadState
        WHERE [Key] LIKE @LikePattern
          AND State = 2
          AND ScalarValue IS NOT NULL
          AND ValueIsNumeric = 1;
    END
    ELSE IF @Agg = 'FIRST'
    BEGIN
        SELECT TOP (1) @R = ScalarValue
        FROM #ThreadState
        WHERE [Key] LIKE @LikePattern
          AND State = 2
          AND ScalarValue IS NOT NULL
    ...
    ELSE IF @Agg = 'CONCAT'
    BEGIN
        SELECT @R = ISNULL(STRING_AGG(ScalarValue, ',') WITHIN GROUP (ORDER BY SeqId), '')
        FROM #ThreadState
        WHERE [Key] LIKE @LikePattern
          AND State = 2
          AND ScalarValue IS NOT NULL;
    ...
    BEGIN TRY
        ...
        SELECT 
            ...
            CASE 
                ...
                THEN (
                    CASE UPPER(j.Aggregator)
                        WHEN 'SUM' THEN (SELECT CAST(SUM(TRY_CAST(ScalarValue AS DECIMAL(38,10))) AS NVARCHAR(MAX))
                                         FROM #ThreadState
                                         WHERE [Key] LIKE CASE WHEN j.IsRuleRef = 1 THEN 'rule:' ELSE '' END + REPLACE(REPLACE(j.Pattern, '*', '%' AND State = 2 AND ScalarValue IS NOT NULL AND ValueIsNumeric = 1)
                        WHEN 'COUNT' THEN (SELECT CAST(COUNT(1) AS NVARCHAR(MAX))
                                           FROM #ThreadState
                                           WHERE [Key] LIKE CASE WHEN j.IsRuleRef = 1 THEN 'rule:' ELSE '' END + REPLACE(REPLACE(j.Pattern, '*', '%' AND State = 2 AND ScalarValue IS NOT NULL AND ValueIsNumeric = 1)
                        WHEN 'AVG' THEN (SELECT CAST(AVG(TRY_CAST(ScalarValue AS DECIMAL(38,10))) AS NVARCHAR(MAX))
                                         FROM #ThreadState
                                         WHERE [Key] LIKE CASE WHEN j.IsRuleRef = 1 THEN 'rule:' ELSE '' END + REPLACE(REPLACE(j.Pattern, '*', '%' AND State = 2 AND ScalarValue IS NOT NULL AND ValueIsNumeric = 1)
                        WHEN 'MIN' THEN (SELECT CAST(MIN(TRY_CAST(ScalarValue AS DECIMAL(38,10))) AS NVARCHAR(MAX))
                                         FROM #ThreadState
                                         WHERE [Key] LIKE CASE WHEN j.IsRuleRef = 1 THEN 'rule:' ELSE '' END + REPLACE(REPLACE(j.Pattern, '*', '%' AND State = 2 AND ScalarValue IS NOT NULL AND ValueIsNumeric = 1)
                        WHEN 'MAX' THEN (SELECT CAST(MAX(TRY_CAST(ScalarValue AS DECIMAL(38,10))) AS NVARCHAR(MAX))
                                         FROM #ThreadState
                                         WHERE [Key] LIKE CASE WHEN j.IsRuleRef = 1 THEN 'rule:' ELSE '' END + REPLACE(REPLACE(j.Pattern, '*', '%' AND State = 2 AND ScalarValue IS NOT NULL AND ValueIsNumeric = 1)
                        WHEN 'FIRST' THEN (SELECT TOP (1) ScalarValue
                                           FROM #ThreadState
                                           WHERE [Key] LIKE CASE WHEN j.IsRuleRef = 1 THEN 'rule:' ELSE '' END + REPLACE(REPLACE(j.Pattern, '*', '%' AND State = 2 AND ScalarValue IS NOT NULL
                                           ORDER BY SeqId ASC)
                        WHEN 'LAST' THEN (SELECT TOP (1) ScalarValue
                                          FROM #ThreadState
                                          WHERE [Key] LIKE CASE WHEN j.IsRuleRef = 1 THEN 'rule:' ELSE '' END + REPLACE(REPLACE(j.Pattern, '*', '%' AND State = 2 AND ScalarValue IS NOT NULL
                                          ORDER BY SeqId DESC)
                        WHEN 'CONCAT' THEN (SELECT ISNULL(STRING_AGG(ScalarValue, ',') WITHIN GROUP (ORDER BY SeqId), '')
                                            FROM #ThreadState
                                            WHERE [Key] LIKE CASE WHEN j.IsRuleRef = 1 THEN 'rule:' ELSE '' END + REPLACE(REPLACE(j.Pattern, '*', '%' AND State = 2 AND ScalarValue IS NOT NULL)
                        ELSE NULL
                    END
