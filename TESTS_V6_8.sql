/***********************************************************************
    TESTS COMPLETS - MOTEUR DE RÈGLES V6.8
    =====================================================================
    
    Conformité: SPEC V1.6.0
    Couverture: 62 tests fonctionnels + 6 benchmarks performance
    ...
    11. Fonctions SQL dans expressions (14 tests)
    12. Tests de performance (6 benchmarks)
    13. RuleRef / Lazy LIKE / Cycles (3 tests) <<< V6.8
************************************************************************/
...
PRINT '======================================================================';
PRINT '        TESTS COMPLETS - MOTEUR DE RÈGLES V6.8                       ';
PRINT '======================================================================';
PRINT '';
...
IF @PassedTests = @TotalTests
    PRINT '  ✅ TOUS LES TESTS PASSENT - MOTEUR V6.8 CONFORME SPEC V1.6.0';
ELSE
...
-- ====================================
-- 13. Tests RuleRef / Lazy LIKE (V6.8)
-- ====================================
PRINT '';
PRINT '--- 13. Tests RuleRef / Lazy LIKE (V6.8) ---';

-- Ajout règles dédiées V6.8 (idempotent)
IF NOT EXISTS (SELECT 1 FROM dbo.RuleDefinitions WHERE RuleCode = 'R_DEP_A')
    INSERT INTO dbo.RuleDefinitions (RuleCode, Expression) VALUES ('R_DEP_A', '10');
IF NOT EXISTS (SELECT 1 FROM dbo.RuleDefinitions WHERE RuleCode = 'R_DEP_B')
    INSERT INTO dbo.RuleDefinitions (RuleCode, Expression) VALUES ('R_DEP_B', '{Rule:R_DEP_A} + 5');

-- Règles pour test Lazy LIKE (préfixe LZR sans underscore pour éviter conflit wildcard)
IF NOT EXISTS (SELECT 1 FROM dbo.RuleDefinitions WHERE RuleCode = 'LZR1')
    INSERT INTO dbo.RuleDefinitions (RuleCode, Expression) VALUES ('LZR1', '1');
IF NOT EXISTS (SELECT 1 FROM dbo.RuleDefinitions WHERE RuleCode = 'LZR2')
    INSERT INTO dbo.RuleDefinitions (RuleCode, Expression) VALUES ('LZR2', '2');
IF NOT EXISTS (SELECT 1 FROM dbo.RuleDefinitions WHERE RuleCode = 'LZRNULL')
    INSERT INTO dbo.RuleDefinitions (RuleCode, Expression) VALUES ('LZRNULL', 'NULL');
IF NOT EXISTS (SELECT 1 FROM dbo.RuleDefinitions WHERE RuleCode = 'LZRSUM')
    INSERT INTO dbo.RuleDefinitions (RuleCode, Expression) VALUES ('LZRSUM', '{SUM(Rule:LZR?)}');

-- RuleRef directe (doit exécuter R_DEP_A automatiquement)
EXEC dbo.sp_Test 'RULE_REF', '13.1 RuleRef direct', 
    '{"rules":["R_DEP_B"]}', 'R_DEP_B', '15';

-- Lazy LIKE + agrégat (doit découvrir/exécuter LZR1/LZR2, pas LZRNULL ni LZRSUM)
-- Pattern LZR? matche LZR1, LZR2 (4 chars) mais pas LZRNULL (7 chars) ni LZRSUM (6 chars)
EXEC dbo.sp_Test 'RULE_REF', '13.2 Lazy LIKE SUM rules', 
    '{"rules":["LZRSUM"]}', 'LZRSUM', '3';

-- Cycle detection
IF NOT EXISTS (SELECT 1 FROM dbo.RuleDefinitions WHERE RuleCode = 'RCYC1')
    INSERT INTO dbo.RuleDefinitions (RuleCode, Expression) VALUES ('RCYC1', '{Rule:RCYC2} + 1');
IF NOT EXISTS (SELECT 1 FROM dbo.RuleDefinitions WHERE RuleCode = 'RCYC2')
    INSERT INTO dbo.RuleDefinitions (RuleCode, Expression) VALUES ('RCYC2', '{Rule:RCYC1} + 1');

EXEC dbo.sp_Test 'RULE_REF', '13.3 Cycle detected', 
    '{"rules":["RCYC1"]}', 'RCYC1', NULL, 0, 1;
...
-- =========================================================================
PRINT '======================================================================';
PRINT '    RAPPORT FINAL V6.8';
PRINT '======================================================================';
PRINT '';

-- Recompter avec les tests V6.8
DECLARE @FinalTotal INT, @FinalPass INT;
SELECT @FinalTotal = COUNT(*), @FinalPass = SUM(CASE WHEN Passed=1 THEN 1 ELSE 0 END) FROM dbo.TestResults;

PRINT '  Tests totaux: ' + CAST(@FinalPass AS VARCHAR) + '/' + CAST(@FinalTotal AS VARCHAR);

IF @FinalPass = @FinalTotal
    PRINT '  ✅ TOUS LES TESTS PASSENT - MOTEUR V6.8 CONFORME';
ELSE
BEGIN
